    ...
    return (
      <Grid fluid>
        <Row center="lg" style={menuRowOneStyle}>
          <Col xs={4}>
            <Avatar src={menuDetail.image} size="60" />
          </Col>
          <Col xs={8} sm={6} lg={4}>
        ...
        <Row center="lg" style={menuRowTwoStyle}>
          {menuDetail.served.map(c => (
            <Chip key={c} backgroundColor={blue300} style={styles.chip}>
              {c}
            </Chip>
          ))}
        </Row>
        <Row>
          <Chip backgroundColor="#2ecc71" style={styles.chip}>
            {menuDetail.spiceLevel}
          </Chip>
        </Row>
      </Grid>
    );
